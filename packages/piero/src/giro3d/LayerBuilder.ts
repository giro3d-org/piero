...
import { GeoJSON, GPX, KML, MVT, WMTSCapabilities } from 'ol/format';
...
import { getConfig, getDynamicStyles } from '@/config-loader';
...
): Promise<ColorLayer | ElevationLayer | MaskLayer> {
    ...
    switch (basemap.type) {
        ...
        case 'elevation': {
            ...
            return new ElevationLayer({
                ...
                minmax: opts.minmax ?? { max: 5000, min: 0 },
                noDataOptions: {
                    replaceNoData: false,
                },
            });
...
async function getLayerOptions(
    layer: BaseLayer | DatasetAsLayerConfig | Overlay,
...
function parseStaticStyle(style: StaticVectorStyle): Style {
    function parseStroke(stroke?: StrokeStyle): Stroke | undefined {
        if (stroke) {
            return new Stroke({
                color: stroke.color,
                width: stroke.width,
    ...
    function parseFill(fill?: FillStyle): Fill | undefined {
        if (fill) {
            return new Fill({
                color: fill.color,
            });
    ...
    function parsePoint(point?: PointStyle): ImageStyle | undefined {
        if (point) {
            return new Circle({
