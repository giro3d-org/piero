...
<template>
  <li class="list-group-item item">
    <div>
      <VisibilityControl class="float-start" :visible="dataset.visible" v-on:update:visible="(v) => $emit('update:visible', v)" />
      <a :class="!dataset.visible ? 'disabled' : null" class="dataset" :title="dataset.name" href="#" @click="$emit('zoom')">{{ dataset.name }}</a>
      <div class="float-end icons">
        <a v-if="dataset.isLoaded" href="#" class="icon" title="Toggle 3D grid" @click="$emit('udpdate:toggle-grid')">
          <i class="bi bi-box"></i>
        </a>
