...
<template>
  <div class="mb-3">
    <DropdownView label="Source" :current="null" :items="store.getCompatibleDatasets()"
      @updated:current="src => setCurrentSource(src as Dataset)" />
  </div>
  <IconButton title="Generate stats..." class="btn-primary" @click="createStatistics" icon="bi-pie-chart-fill"
    text="Generate" />
